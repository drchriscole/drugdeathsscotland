...
#  labs(caption="Males are mostly affected by drug-related deaths, but female deaths have increased since 2000") +
...
#  labs(caption = "In 2000 deaths were largely in the under 35s.\nIn 2018 they are dominated by the 35 and over. Particluarly in females.") +
  ylab('Percentage Deaths') +
  geom_col() +
  facet_grid(Sex ~ .) +
...
change.dat = data.frame(Region = dat$Region, Change = dat$`2018` - dat$`2017`, PcChange = round(100*(dat$`2018` - dat$`2017`)/dat$`2017`,1))
...
ggplot(change.dat, aes(x=Region, y=Change, fill="#D55E00")) +
  ggtitle("2018 Change in Drug-Related Deaths", subtitle="By NHS health board") +
  ylab("Change in No. Deaths") +
  xlab('') +
  geom_col() +
  ylim(c(-15,150)) +
  coord_flip() +
...
names(dat) = c('Council',seq(2008,2018),'Pop')

# calc DRD rate
dat = cbind(dat, Rate = 1000000*dat$`2018`/dat$Pop)
...
ggplot(dat, aes(x=Council, y=Rate, fill=factor(ifelse(dat$Council=="Dundee City", "Y","N")))) + 
  geom_col() +
  ylim(c(0,480)) +
  ggtitle("Drug-Related Deaths By Council Area", subtitle = "Per million population") +
  ylab("Drug-Related Deaths (per million population)") +
  labs(caption="Using 2016 population data") +
  ...
  annotate("text", label=sprintf("Scotland (%.1f)",scot.rate), x=5.5, y=scot.rate+8,colour="grey", angle=-90) +
  coord_flip() +
  theme_minimal() +
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
...
dat$Council = c("Aberdeen", "Abrdshr", "Angus", "Argyll", "Edinburgh", "Clk", "DnG", "Dundee", "E. Ayr", "E. Dun", "E. Loth", "E. Ren", "Falkirk", "Fife", "Glasgow", "Highland", "Inverclyde", "Midlothian", "Moray", "Na h-Eil", "N. Ayr", "N. Lankshr", "Orkney", "PnK", "Renfrshr", "SBord", "Shetld", "S. Ayr", "S. Lankshr", "Stirling", "W. Dun", "West Loth")
...
ggplot(dat.pc.m, aes(x=dat.Council, y=value, fill=factor(ifelse(dat.Council=="Dundee", "Y","N")))) +
  ggtitle("Drug Implicated in Death", subtitle="For councils with >40 deaths") +
  xlab("Council") +
  ylab("Percentage Deaths") +
  geom_col() +
...
ggplot(dat.m, aes(x=variable, y=value, group=Council, colour=ifelse(Council == "Dundee", "A","B"))) +
  ggtitle("Drug-Related Deaths per 1,000 population", subtitle = "By council area (annual average 2014-2018)") +
  ylab("Death Rate") +
  xlab("Age Group") +
  geom_line() +
  ...
  geom_text(aes(label=round(value,2)), vjust = -0.6, size=2.4, colour="black") +
  facet_grid(Council ~.) +
