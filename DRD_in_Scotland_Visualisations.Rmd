...
# Drug-Related Deaths in Scotland 2020 {.tabset}
...
A collection of visualisations of the [2020 data](https://www.nrscotland.gov.uk/files//statistics/drug-related-deaths/20/drug-related-deaths-20-tabs-figs.xlsx) are presented here for those interested in exploring the trends or patterns in the data. Little to no commentary is provided on what the data show.
...
colnames(dat) = c('Region', seq(startyr,endyr,1))
...
ggplot(dat, aes(x=Region, y=`2020`)) +
  ggtitle("Drug-Related Deaths in Scotland", subtitle="Change between 2010 - 2020") +
  geom_linerange(aes(ymin=`2010`,ymax=`2020`), colour='grey', size=1.5) +
  geom_point(aes(y=`2010`), size =1.2, colour='orange') +
  geom_point(aes(y=`2020`), size =1.2, colour='orange') +
  geom_point(aes(y=`2019`), size =1.2, colour='darkgreen') +
  geom_text(aes(y=`2019`, label = ifelse(Region == "Greater Glasgow & Clyde", '2019', '')), vjust = 1.4, size=2.2, colour="grey30") +
  geom_text(aes(y=`2010`, label = ifelse(Region == "Greater Glasgow & Clyde", '2010', '')), vjust = -0.5, size=2.6, colour="grey30") +
  geom_text(aes(y=`2020`, label = ifelse(Region == "Greater Glasgow & Clyde", '2020', '')), vjust = -0.5, size=2.6, colour="grey30") +
  ylab('No. Deaths') +
  xlab('') +
  coord_flip() +
  theme_minimal()
...
change.dat = data.frame(Region = dat$Region, Change = dat$`2020` - dat$`2019`, PcChange = round(100*(dat$`2020` - dat$`2019`)/dat$`2019`,1))
...
colnames(dat) = c('Council',seq(startyr,endyr,1))
...
ggplot(dat.m, aes(x=variable, y=value, group=Council, colour=ifelse(dat.m$Council == "Dundee", "A","B"))) +
  ggtitle(sprintf("Drug-Related Deaths in Scotland by Council %d-%d",startyr,endyr)) +
  xlab('Year') +
  ylab('No. Deaths') +
  geom_line(size=1.1) +
  scale_x_discrete(breaks=c(startyr, endyr), labels=c("'10","'20")) +
...
names(dat) = c('Council',seq(startyr,endyr),'Pop')

# calc DRD rate
dat = cbind(dat, Rate = 1000000*dat$`2020`/dat$Pop)
...
ggplot(dat, aes(x=Council, y=Rate, fill=factor(ifelse(dat$Council=="Dundee City", "Y","N")))) +
  geom_col() +
  ylim(c(0,490)) +
  ggtitle(sprintf("Drug-Related Deaths in Scotland %d", endyr), 
...
ggplot(dat.num.l, aes(x = Year, y = as.numeric(Counts), group = Method, colour = Method)) +
  geom_line() +
  geom_point(data = dat.num.l %>% filter(Year == '2020')) +
  geom_text(data = dat.num.l %>% filter(Year == '2020'), aes(label = as.numeric(Counts)), colour = 'grey4', size = 2.4, hjust = -0.2) +
  scale_color_manual(values = myPalette) +
#  scale_x_discrete(breaks = ybreaks) +
  labs(title = 'Drug-Related Deaths in Scotland',
